...
def create_recipe(request):
    ...
    if request.method == 'POST':
        ...
        new_recipe = CreatedRecipe.objects.create(
            ...
            ready_in_minutes=(int(ready_in_minutes)
                              if ready_in_minutes else None),
            featured_image=featured_image
        )
        messages.success(
            request, 'Your recipe has been created successfully!')
...
def edit_created_recipe(request, recipe_id):
    ...
    if request.method == 'POST':
        ...
        recipe.save()
        messages.success(
            request, 'Your recipe has been updated successfully!')
...
def share_created_recipe(request, recipe_id):
    ...
    if request.method == 'POST':
        ...
        # Unique identifier for created recipes, won't clash with API IDs
        # Will have to be handled specially in other views for modularity
        recipe_obj, created = Recipe.objects.get_or_create(
            recipe_id=f"created_{recipe.id}",
            defaults={
                'title': recipe.title,
                'image_url': (recipe.featured_image.url
                              if recipe.featured_image else None),
        ...
        messages.success(
            request,
            f'"{recipe.title}" has been shared to the community feed!')
    ...
    try:
        ...
        try:
            ...
            user_recipe = UserRecipe.objects.get(
                user=request.user, recipe=recipe_obj)
        ...
        messages.success(
            request,
            f'"{recipe.title}" has been removed from the community feed.')
    except CreatedRecipe.DoesNotExist:
        messages.error(request, 'Recipe not found.')
